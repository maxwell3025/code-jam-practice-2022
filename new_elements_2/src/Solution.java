...
public class Solution{
    ...
    //Problem Variables
    int N;
    List<Pair<Integer, Integer>> compounds = new ArrayList<>();
    //Solution Variables
    ...
    public Solution(int CASE){
        ...
        for(int i = 0;i<N;i++){
            int c = in.nextInt();
            int j = in.nextInt();
            compounds.add(new Pair<>(c, j));
        }
        //Solve Problem
        for(Pair<Integer, Integer> a: compounds){
            for(Pair<Integer, Integer> b: compounds){
                int dc = b.first-a.first;
                int dj = b.second-a.second;
                if((long)dc * dj < 0){
                    if(dc>0){
                        Rational newMin = new Rational(-dj, dc);
                        if(newMin.compareTo(min) > 0){
                            min = newMin;
                        }
                    }else{
                        Rational newMax = new Rational(dj, -dc);
        ...
        System.out.println("Case #" + CASE + ": " + c + " " + j);
    }
